...
namespace Synergy.ReliefCenter.Services
{
    public class AuthorizationPolicyService : IAuthorizationPolicyService
    {
        ...
        public AuthorizationPolicyService(IAccessPoliciesRepository accessPoliciesRepository)
        {
        ...
        public bool ValidateRole(string token, string policy)
        {
            try
            {
                ...
                var allowedRoles = _accessPoliciesRepository.GetAllowedRoles(policy,tokenUserId);
                if (allowedRoles != null)
                {
                    ...
                    //        if (roleList != null)
                    //        {
                    //            return (users.Count() > 0 ? _policyUsersRepository.GetAllIncluding().Where(w => w.AccessPolicyId.Equals(policyList.Id)
                    //            && w.UserId.Equals(tokenUserId)).Count() > 0 : true) && (roleList.AllowedRanks != null ?
                    //            roleList.AllowedRanks.First().Equals(tokenRank) : true) ? true : false;
                    //        }
                    //    }
                    //}
                   return false;
